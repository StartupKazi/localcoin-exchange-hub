...
const marqueeMessages = [
  "⚠️ Please be cautious of scammers who may share personal contact details, such as phone numbers or email addresses. Always verify information independently and do not rely on screenshots.",
  ...
  "🛡️ Never share your account password, 2FA codes, or private keys with anyone. LocalCoin Trade support will never ask for these details.",
...
const chainedText = marqueeMessages.join("     ●     ");
...
const MarqueeAnnouncement = () => (
  <div className="px-4 py-2.5 border-t border-b border-border/40 flex items-center gap-2 text-sm text-muted-foreground overflow-hidden">
    <Volume2 className="h-4 w-4 shrink-0 z-10" />
    <div className="overflow-hidden whitespace-nowrap flex-1">
      <div className="inline-flex animate-marquee">
        <span className="whitespace-nowrap px-4">{chainedText}</span>
        <span className="whitespace-nowrap px-4">{chainedText}</span>
      </div>
    </div>
  </div>
...
const TradeTable = ({
  ...
  return (
    <div>
      ...
      {/* Table Header */}
      <div className="hidden md:grid grid-cols-[2fr_1.2fr_2fr_2fr_1.2fr] px-4 py-3 text-xs text-muted-foreground border-b border-border/30">
          ...
          <div key={offer.id} className="px-4 py-4">
            ...
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-sm font-semibold text-foreground truncate">{offer.merchant}</span>
                <Shield className="h-3.5 w-3.5 text-primary shrink-0" />
              </div>
              <div className="flex items-center gap-1 text-xs text-muted-foreground shrink-0">
                <Clock className="h-3 w-3" /> {offer.time}
              </div>
            </div>
            ...
            {/* Price */}
            <div className="mb-2">
              <span className="text-xs text-muted-foreground mr-1">{offer.currency}</span>
              <span className="text-2xl font-bold text-foreground">{offer.price}</span>
            </div>
            ...
            <div className="text-sm text-muted-foreground">
              <span>Available </span>
              <span className="text-foreground">{offer.available}</span>
            </div>
            <div className="text-sm text-muted-foreground mb-3">
              <span>Limits </span>
              <span className="text-foreground">{offer.limit}</span>
            </div>
